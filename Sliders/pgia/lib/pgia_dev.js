(function() {
    ...
    //Update _createMarker in GSAP, search for space:nowrap;font-family:sans-serif,Arial
    ...
    var PgClassPlugin = {
        ...
        init: function init(target, value, tween, index, targets) {
            ...
            this.cid = cid++;
            //console.log('init class', this.cid, target, value);
        },
    ...
    var PgDOMPlugin = {
        ...
        init: function init(target, value, tween, index, targets) {
            ...
            this.target_element = typeof value.target_element == 'function' ? value.target_element(target) : (value.target_element || null);
            ...
            if(data.template) {
                ...
                while(c > data.created.length) {
                    ...
                    var template = evaluateTemplateString(el.innerHTML, function(a) {
                        changed = true;
                        return (data.target_element || el).getAttribute(a) || '';
                    });
                    ...
                    if(data.v.pos) {
                        ...
                        el._pg_position = new PgPosition(el, data.target_element,
                            evaluateParam(data.v.pos, el, data.target_element) || 'auto',
    ...
    var PgPositionPlugin = {
        ...
        init: function init(target, value, tween, index, targets) {
            this.t = target;
            this.v = value;
            var te = typeof value.target_element == 'function' ? value.target_element(target) : (value.target_element || null);
            this.pos = new PgPosition(target, te,
                evaluateParam(value.pos || 'auto', target, te),
                evaluateParam(value.dist || '0', target, te),
                evaluateParam(value.width || '0', target, te),
                evaluateParam(value.height || '0', target, te)
            );
        },
        render: function render(ratio, data) {
            data.pos.move(ratio);
    ...
    function getVariableValue(name, node, target_el, def) {
        ...
        for(var i = start; i < aa.length; i++) {
            ...
            if(typeof val == 'object') {
                if(val instanceof HTMLElement && field == 'style' && i < aa.length - 1) {
                    val = window.getComputedStyle(val).getPropertyValue(aa[i+1]);
                    return val === null ? def : val;
                }
                if('hasAttribute' in val && val.hasAttribute(field)) {
                    val = val.getAttribute(field);
                } else if(field in val) {
                    val = typeof val[field] == 'function' ? val[field].call(val) : val[field];
                    console.log(field, val);
                    if(func && !isNaN(val) && (func in Math)) {
                        val = Math[func](pf(val));
                    }
                    console.log(field, val);
                } else return def;
            } else {
    ...
    function evaluatePercent(value, base) {
        var r = (value + '').replace(/([0-9]+)\%/, function(m, val) {
            base = typeof base === 'function' ? base() : base;
            return base * pf(val) / 100.0;
        })
        return pf(r, r);
    }
...
*/
    ...
    var PgCustomAnimation = function () {
        ...
        this.getTimeline = function (d, element, done) {
            ...
            var target_element_key = '_pgia_target_' + timeline_count++;

            var getTargetElement = function(el, el2) { //this can also be called from GSAP resolver
                return (el2 ? el2[target_element_key] : el[target_element_key]) || null;
            }
                        ...
                        list.forEach(function (t) {
                            ...
                            if (el.t) {
                                ...
                                forEachTarget(element, function (el_node) {
                                    ...
                                    Array.prototype.forEach.call(getTargets(el_node, el.t), function (node) {
                                        if (nodes.indexOf(node) < 0) {
                                            nodes.push(node);
                                            node[target_element_key] = el_node;
                                        }
                                    });
                                })
                            }
                            ...
                            var map = {
                                ...
                                'pgia.play': function (p, v) {
                                    on_s.push(function () {
                                        nodes.forEach(function (n) {
                                            v && pgia.play(n, evaluateParam(v, n, getTargetElement(n)));
                                ...
                                'pgia.pause': function (p, v) {
                                    on_s.push(function () {
                                        nodes.forEach(function (n) {
                                            v && pgia.pause(n, evaluateParam(v, n, getTargetElement(n)), tl);
                                            //v && n._pg_animations && n._pg_animations.play(parseInt(v)-1);
                                        })
                                    });
                                ...
                                scrollTo: function (p, v) {
                                    ...
                                    var f = function (i, target) {
                                        var target_element = getTargetElement(target);
                                        ...
                                        switch (evaluated_dest) {
                                            ...
                                            default:
                                                if (startsWith(evaluated_dest, 'item')) {
                                                    ...
                                                    d[dir] = evaluatePercent(evaluated_dest, function() {
                                                        var dim = dir == 'y' ? 'Height' : 'Width';
                                                        return target['client' + dim];
                                                    })
                                                    /*
                                ...
                                pgPos: function (p, v) {
                                    ...
                                    v.target_element = getTargetElement;
                                    props.pgPos = v;
                                },
                                ...
                                'history.push': function (p, v) {
                                    on_s.push(function () {
                                        nodes.forEach(function (n) {
                                            v = evaluateParam(v, n, getTargetElement(n));
                            ...
                            var getValueOrFunction = function(v) {
                                if (typeof v == 'string' && v.indexOf('${') >= 0) {
                                    var template = v;
                                    return function (i, node, list) {
                                        return evaluateParam(template, node, getTargetElement(node));
                                    }
                                }
                                return v;
                            }

                            map['stroke-dasharray'] = map['stroke-dashoffset'] = function(p, v) {
                                v = getValueOrFunction(v);

                                props[p] = function(i, n, list) {
                                    (typeof v == 'function') && (v = v(i, n, list));
                                    return evaluatePercent(v, function() {
                                        return n.getTotalLength ? n.getTotalLength() : 100;
                                    })
                                }
                            }
            ...
            try {
                ...
                apply_attrs.forEach( function(attr) {
                    if(el.hasAttribute(attr)) {
                        ...
                        list.push({a: attr, v: val, d: parseJson(val)});
                    }
                });
...
//            console.log(`start = ${start}, end = ${end}`);
            var update_was_triggered = false;
            ...
            var scrollOptions = {
                ...
                onRefresh: function(self) {
                    //console.log('st refresh, active ', self.isActive);
                    /*
                    var scroller = self.scroller;

                    if(false && scroller && !update_was_triggered) {
                        var scroller_pos = self.vars.horizontal ? (scroller.pageXOffset || scroller.scrollLeft || 0) : (scroller.pageYOffset || scroller.scrollTop || 0);
                        if(scroller_pos === self.start || (self.pin && scroller_pos - self.start < 0.01)) {
                            //start position is visible, let's force animation update
                            //force update of the timeline on scene create, so that starting values and so on can be recorder and set
                            tl.seek(0.1);
                            tl.seek(0);
                        }
                    }*/
            ...
            var onUpdate = function(st) {
                st = this.pgPinScrollTrigger || this.scrollTrigger;
                //console.log("progress:", this.progress(), "direction:", st.direction, "velocity", st.getVelocity(), 'start', st.start);
                _this.seek(this.progress(), st.direction);
                if(scene_id) {
            ...
            if(scrollOptions.pin) {
                //need a workaround for -0.001 start position
                var onPinUpdate = function() {
                    var t = this.progress() * tl.duration();
                    //console.log("PIN time:", t);
                    if(t < 0.001) t = 0;
                    tl.seek(t, false);
                }

                /*
                scrollOptions.onUpdate = function(self) {
                    if(!update_was_triggered) {
                        if(self.start === -0.001) {
                            tl.seek(0.1);
                            tl.seek(0);
                        }
                ...
                */
                ...
                scene = tl.pgPinScrollTrigger = pin_tl.scrollTrigger;

                pin_tl.to({}, 100, {}, 0);

                tl.pause();
                gsap.globalTimeline.add(tl);
